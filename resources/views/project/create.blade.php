<x-master-layout>
    ...
    <form action="{{ route('admin.projects.store') }}" method="post">
        <x-card title="Projek Baru" footer='<a href="{{ url()->previous() }}" class="btn btn-secondary me-2">Cancel</a><button type="submit" class="btn btn-primary">Submit</button>'>
            @csrf
            <!--begin::Input group-->
            <div class="mb-6 row">
                <!--begin::Label-->
                ...
                <div class="col-lg-8 fv-row fv-plugins-icon-container">
                    <input type="text" name="project_name" value="{{ old('project_name') }}" class="form-control form-control-lg form-control-solid">
                    @error('project_name')
            ...
            <div class="mb-6 row">
                ...
                <!--begin::Col-->
                <div class="col-lg-8 fv-row fv-plugins-icon-container">
                    <textarea name="description" class="form-control form-control-lg form-control-solid" rows="6">{{ old('description') }}</textarea>
                    @error('description')
                        <div class="fv-plugins-message-container invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="mb-6 row">
                ...
                <!--begin::Col-->
                <div class="col-lg-8 fv-row fv-plugins-icon-container">
                    <input type="number" name="year" value="{{ old('year') }}" class="form-control form-control-lg form-control-solid">
                    @error('year')
                        <div class="fv-plugins-message-container invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-lg-8 fv-row fv-plugins-icon-container">
                    <select name="kluster" class="form-select form-select-lg form-select-solid" data-control="select2" data-placeholder="Pilih Kluster" data-allow-clear="true">
                        ...
                        @foreach ($categories as $category)
                            <option value="{{ $category->id }}" {{ $category->id == old('kluster') ? 'selected' : '' }}>
                ...
                <div class="col-lg-8 fv-row fv-plugins-icon-container">
                    <select name="users[]" class="form-select form-select-lg form-select-solid" data-control="select2" data-placeholder="Select users" data-allow-clear="true" multiple="multiple">
                        ...
                        @foreach ($users as $user)
                            <option value="{{ $user->id }}" {{ in_array($user->id, old('users', [])) ? 'selected' : '' }}>{{ $user->name }}
                            </option>
                        @endforeach
                    </select>
                    @error('users')
                        ...
                        <div class="col-lg-6 fv-row fv-plugins-icon-container">
                            <div class="form-check form-check-custom form-check-solid mb-2">
                                <input class="form-check-input" name="folder_types[]" type="checkbox" value="{{ $type->id }}"
                                    {{ in_array($type->id, old('folder_types', [])) ? 'checked' : '' }} />
            ...
            <div class="mb-6 row">
                ...
                <!--end::Label-->
                <div class="col-lg-8 fv-row fv-plugins-icon-container">
                    <input type="date" class="form-control form-control-lg form-control-solid" id="dateInput" name="date">
                    @error('date')
                        <div class="fv-plugins-message-container invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
